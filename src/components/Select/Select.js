...
const Select = ({ items, onChange, error, label, alignment = "left", variant = "default" }) => {
  ...
  return (
    <S.SelectWrapper variant={variant}>
      ...
      <S.Select variant={variant} ref={ref}>
        <S.SelectAnchor onClick={onToggleMenu}>{items[selectedIndex].label}</S.SelectAnchor>
        {isShown && (
          <S.SelectMenu alignment={alignment}>
            {items.map((item, index) => (
              <S.SelectItem key={item.label} onClick={() => handleClick(index)}>
                {item.label}
              </S.SelectItem>
            ))}
          </S.SelectMenu>
